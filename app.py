...
if not st.session_state.connecte:
    ...
    st.markdown(
        "<h2 style='text-align: center;'>Connexion ENGITAS</h2>",
        unsafe_allow_html=True,
    )
...
# --- SI L'UTILISATEUR EST CONNECTÉ ---
    ...
    with st.sidebar:
        st.markdown("### **ENGITAS**")
        st.markdown(
            f"👤 **Connecté :** <span style='color: #38bdf8;'>{st.session_state.username}</span>",
            unsafe_allow_html=True,
        )
        st.markdown("---")
        ...
        for i, emp in enumerate(st.session_state.employes_list):
            ...
            with col3:
                if st.button("Enregistrer", key=f"btn_{i}"):
                    st.session_state.employes_list[i][
                        "poste"
                    ] = nouveau_poste
...
# --- PIED DE PAGE ---
st.markdown(
    '<div class="footer">Design & Développement by <b>ARNOLD OMAM</b> | ENGITAS 2026</div>',
    unsafe_allow_html=True,
)